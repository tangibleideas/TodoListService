...
class TodoItemServiceTest {
  ...
  void setUp() {
    ...
    todoItemDTO =
        new TodoItemDTO(
            ...
            FIXED_NOW_INSTANT);
    addTodoItemRequest =
        new AddTodoItemRequest("Test Description", FIXED_NOW_INSTANT.plusSeconds(SECONDS_DELAY_1));
  }
  ...
  class FindTodoItemByIdTests {
    ...
    void shouldFindTodoItemById() {
      when(todoItemRepository.findById(1L)).thenReturn(Optional.of(todoItem_Id1));
  ...
  class AddTodoItemTests {
    ...
    void shouldAddItemSuccessfully() {
      when(todoItemMapper.fromAddTodoItemRequest(addTodoItemRequest)).thenReturn(todoItem_Id1);
      ...
      verify(todoItemMapper, times(1)).fromAddTodoItemRequest(addTodoItemRequest);
  ...
  class UpdateItemTests {
    ...
    void shouldUpdateDescription() {
      ...
      UpdateTodoItemRequest request = new UpdateTodoItemRequest(newDescription, null);
      ...
      Optional<TodoItemDTO> result = todoItemService.updateItem(1L, request);
      // ASSERT
      assertThat(result).isPresent();
    ...
    void shouldUpdateStatusToDone() {
      UpdateTodoItemRequest request = new UpdateTodoItemRequest(null, TodoItemStatus.DONE);
      Instant timeWhenUpdateHappened = Instant.now();
      ...
      when(todoItemRepository.findById(1L)).thenReturn(Optional.of(todoItem_Id1));
      when(todoItemRepository.save(any(TodoItem.class))).thenReturn(updatedTodoItem);
      when(todoItemMapper.toDTO(any(TodoItem.class))).thenReturn(updatedDto);

      Optional<TodoItemDTO> result = todoItemService.updateItem(1L, request);

      assertThat(result).isPresent();
      assertThat(result.get().status()).isEqualTo(TodoItemStatus.DONE);
      assertThat(result.get().doneAt()).isNotNull();
      verify(todoItemRepository, times(1)).save(any(TodoItem.class));
    }

    @Test
    void shouldUpdateStatusToNotDone() {
      UpdateTodoItemRequest request = new UpdateTodoItemRequest(null, TodoItemStatus.NOT_DONE);
      Instant timeWhenUpdateHappened = Instant.now();
      ...
      when(todoItemRepository.findById(1L)).thenReturn(Optional.of(initialTodoItem));
      when(todoItemRepository.save(any(TodoItem.class))).thenReturn(updatedTodoItem);
      when(todoItemMapper.toDTO(any(TodoItem.class))).thenReturn(updatedDto);
      ...
      assertThat(result).isPresent();
      assertThat(result.get().status()).isEqualTo(TodoItemStatus.NOT_DONE);
      assertThat(result.get().doneAt()).isNull();
      verify(todoItemRepository, times(1)).save(any(TodoItem.class));
    }
    ...
    void shouldNotSaveIfNoChanges() {
      UpdateTodoItemRequest request =
          new UpdateTodoItemRequest("Test Description", TodoItemStatus.NOT_DONE);
      TodoItemDTO existingDto = todoItemDTO;

      when(todoItemRepository.findById(1L)).thenReturn(Optional.of(todoItem_Id1));
      when(todoItemMapper.toDTO(any(TodoItem.class))).thenReturn(existingDto);
    ...
    void shouldReturnEmptyOptionalWhenItemToUpdateNotFound() {
      UpdateTodoItemRequest request = new UpdateTodoItemRequest("New Description", null);
      when(todoItemRepository.findById(99L)).thenReturn(Optional.empty());

      Optional<TodoItemDTO> result = todoItemService.updateItem(99L, request);
  ...
  class GetNotDoneItemsTests {
    ...
    void shouldReturnNotDoneItems() {
      ...
      when(todoItemRepository.findAllByStatus(TodoItemStatus.NOT_DONE)).thenReturn(notDoneItems);
      ...
      verify(todoItemRepository, times(1)).findAllByStatus(TodoItemStatus.NOT_DONE);
