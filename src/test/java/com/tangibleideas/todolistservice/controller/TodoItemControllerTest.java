...
class TodoItemControllerTest {
  ...
  @MockitoBean private TodoItemService todoItemService;
  ...
  private final TodoItemDTO todoItemDTO_Id1 =
      new TodoItemDTO(
          1L,
  ...
  void whenGetTodoItemById_thenReturnTodoItem() throws Exception {
    when(todoItemService.findItemById(1L)).thenReturn(Optional.of(todoItemDTO_Id1));

    mockMvc
        .perform(
            MockMvcRequestBuilders.get(API_V1_TODOS + "/1").contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath(Constants.JsonPaths.TODO_ITEM_ID).value(1))
        .andExpect(
            jsonPath(Constants.JsonPaths.TODO_ITEM_DESCRIPTION).value(DESCRIPTION_TODO_ITEM_1))
        .andExpect(
            jsonPath(Constants.JsonPaths.TODO_ITEM_STATUS)
                .value(TodoItemStatus.NOT_DONE.getLabel()))
        .andExpect(
            jsonPath(Constants.JsonPaths.TODO_ITEM_CREATED).value(FIXED_NOW_INSTANT.toString()))
        .andExpect(
            jsonPath(Constants.JsonPaths.TODO_ITEM_DUE)
                .value(FIXED_NOW_INSTANT.plusSeconds(SECONDS_DELAY_1).toString()));
  }
  ...
  void whenGetTodoItemByIdWithWrongIdFormat_thenReturn400() throws Exception {
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.get(API_V1_TODOS + "/abc")
  ...
  void whenAddTodoItem_thenReturnCreated() throws Exception {
    ...
    AddTodoItemRequest addTodoItemRequest =
        new AddTodoItemRequest(
            DESCRIPTION_TODO_ITEM_1, FIXED_NOW_INSTANT.plusSeconds(SECONDS_DELAY_1));

    mockMvc
        .perform(
            MockMvcRequestBuilders.post(API_V1_TODOS)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(addTodoItemRequest)))
  ...
  @Test
  void whenAddTodoItemWithInvalidRequestBody_thenReturn400() throws Exception {
    when(todoItemService.addItem(any())).thenReturn(todoItemDTO_Id1);
  ...
  @Test
  void whenAddTodoItemWithTooLongDescription_thenReturn400() throws Exception {
    when(todoItemService.addItem(any())).thenReturn(todoItemDTO_Id1);

    String tooLongDescription = "x".repeat(MAX_DESCRIPTION_LENGTH + 1);
    AddTodoItemRequest addTodoItemRequest = new AddTodoItemRequest(tooLongDescription, null);
  ...
  @Test
  void whenAddTodoItemWithWrongDueDateFormat_thenReturn400() throws Exception {
    when(todoItemService.addItem(any())).thenReturn(todoItemDTO_Id1);
  ...
  void whenUpdateTodoItemDescription_thenReturnUpdated() throws Exception {
    when(todoItemService.updateItem(any(), any())).thenReturn(Optional.of(todoItemDTO_Id1));

    mockMvc
        .perform(
            MockMvcRequestBuilders.patch(API_V1_TODOS + "/1")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    objectMapper.writeValueAsString(
                        new UpdateTodoItemRequest(DESCRIPTION_TODO_ITEM_1, null))))
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.patch(API_V1_TODOS + "/1")
                ...
                .content(
                    objectMapper.writeValueAsString(
                        new UpdateTodoItemRequest(null, TodoItemStatus.NOT_DONE))))
        .andExpect(status().is(HttpStatus.OK.value()))
        .andExpect(jsonPath(Constants.JsonPaths.TODO_ITEM_ID).value(1))
        .andExpect(
            jsonPath(Constants.JsonPaths.TODO_ITEM_DESCRIPTION).value(DESCRIPTION_TODO_ITEM_1))
  ...
  void whenUpdateTodoItemNotFound_thenReturn404() throws Exception {
    when(todoItemService.updateItem(any(), any())).thenReturn(Optional.empty());
  ...
  @Test
  void whenGetTodoItemsNotDone_thenReturnListByCallingGetNotDoneItems() throws Exception {

    when(todoItemService.getAllItems()).thenReturn(List.of(todoItemDTO_Id1));
    mockMvc
        .perform(MockMvcRequestBuilders.get(API_V1_TODOS).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().is(HttpStatus.OK.value()))
  ...
  @Test
  void whenGetTodoItemsAll_thenReturnListByCallingGetAllItems() throws Exception {
    when(todoItemService.getAllItems()).thenReturn(List.of(todoItemDTO_Id1));
    mockMvc
        .perform(
            MockMvcRequestBuilders.get(API_V1_TODOS + "?all=true")
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().is(HttpStatus.OK.value()))
        .andExpect(jsonPath("$").isArray());
