...
public interface TodoItemRepository extends JpaRepository<TodoItem, Long> {
  ...
  @Modifying
  @Query(
      "UPDATE TodoItem t SET t.status = :newStatus WHERE t.id = :id AND t.status = :currentStatus")
  int updateStatusWithStatusCondition(
      Long id, TodoItemStatus newStatus, TodoItemStatus currentStatus);
}
