...
public class TodoItemService {
  ...
  /**
   * Add TodoItem to database. No custom logic, simply map DTO to Entity and save in the database
   * using repository
   */
  ...
  /**
   * Update TodoItem based on request only if database object requires update 1. Fetch item from
   * database -> 2. Check if there are any updates -> 3. Only save to database if there are updates
   * and updates are allowed
   */
  @Transactional
  public Optional<TodoItemDTO> updateItem(Long id, UpdateTodoItemRequest request) {
    return todoItemRepository
        ...
        .map(
            existingTodoItem -> {
              ...
              if (isDescriptionUpdateRequired(request, existingTodoItem)) {
              ...
              if (isUpdated) {
                ...
                log.debug(
                    "TodoItem with id {} already up to date, " + "skipping database write.", id);
  ...
  private static boolean isDescriptionUpdateRequired(
      UpdateTodoItemRequest request, TodoItem existingTodoItem) {
    return request.description() != null
        && !Objects.equals(request.description(), existingTodoItem.getDescription());
  }

  private static boolean isStatusUpdateRequired(
      UpdateTodoItemRequest request, TodoItem existingTodoItem) {
    return request.status() != null
        && !Objects.equals(request.status(), existingTodoItem.getStatus());
  }
  ...
  private void rejectStatusUpdateIfNeeded(TodoItem existingTodoItem) {
    if (existingTodoItem.getStatus().equals(TodoItemStatus.PAST_DUE)) {
      throw new StatusUpdateNotAllowedException(
          "status update of past due item " + existingTodoItem.getId() + " is not allowed");
    }
  }
  ...
  public List<TodoItemDTO> getNotDoneItems() {
    return todoItemMapper.toDTOs(todoItemRepository.findAllByStatus(TodoItemStatus.NOT_DONE));
  }

  /**
   * Logic connected with status changes. 1. When status changes to done, also set doneAt. 2. When
   * status changes to not done, set doneAt to null.
   */
  ...
  public List<Long> getPastDueTodoItemIds() {
    return todoItemRepository
        .findByStatusAndDueAtBefore(TodoItemStatus.NOT_DONE, Instant.now())
        .stream()
  ...
  public void setPastDueIfNotDone(Long id) {
    int result =
        todoItemRepository.updateStatusWithStatusCondition(
            id, TodoItemStatus.PAST_DUE, TodoItemStatus.NOT_DONE);
    if (result < 1) {
      log.warn(
          "TodoItem with id {} not marked as PAST_DUE as current " + "status not NOT_DONE", id);
    }
  }
}
